...
# Study name ----
study <- "2019-08_habitat-example"  # use this study name to run the example, or change to your own (you will need to save your files with this prefix - see naming of example files)

## Set your working directory ----
working.dir<-dirname(rstudioapi::getActiveDocumentContext()$path) # to directory of current file - or type your own

## Save these directory names to use later----
raw.dir<-paste(working.dir,"example raw data",sep="/") # links to folder called 'example raw data'
tidy.dir<-paste(working.dir,"example tidy data",sep="/") # links to folder called 'example tidy data'
...
# Read in metadata----
metadata <- read_csv(file=paste(study,"metadata.csv",sep = "_"),na = c("", " ")) %>% # read in the file
...
wrong.number<-number.of.annotations%>%
  filter(!number.of.annotations==20) # see images where there is too many or too little annotations (in this example there are none), go back into the *.TMObs file to fix this before re-exporting DO NOT FIX IN THE TXT FILE
